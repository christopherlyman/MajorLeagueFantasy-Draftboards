...
def render_board_html(
    ...
    # Column order comes from draft_order_team_keys_by_slot.
    ...
    if isinstance(draft_order_team_keys_by_slot, list) and len(draft_order_team_keys_by_slot) > 0:
        team_keys = [str(x or "") for x in draft_order_team_keys_by_slot]  # keep blanks
    else:
        # Fallback: best-effort columns from whatever teams dict order is
        team_keys = default_team_keys

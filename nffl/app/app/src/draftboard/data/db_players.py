...
def _map_pos(s: str) -> Position | None:
    ...
    # Yahoo examples include baseball and football positions.
    if t in ("UTIL", "W/R/T", "W/T", "Q/W/R/T", "FLEX", "SUPER FLEX", "SUPERFLEX", "BN", "IR"):
...
def load_active_available_players(dsn: str) -> Dict[str, Player]:
    ...
    Source selection is deployment/env driven so league behavior
    is not hardcoded in code.
    """
    import os

    source = str(
        os.environ.get("DRAFTBOARD_AVAILABLE_PLAYERS_SOURCE", "legacy")
    ).strip().lower()

    if source == "league_pool":

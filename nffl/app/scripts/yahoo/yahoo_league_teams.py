...
def _upsert_yahoo_team_map(payload: dict, league_key: str, season_year: int, dsn: str):
    ...
    try:
        ...
        raise SystemExit(f"Invalid EXPECTED_MANAGER_COUNT/MANAGER_COUNT: {expected_raw!r}")

    if expected_count <= 0:
        raise SystemExit(f"Invalid expected team count: {expected_count}")

    if len(rows) != expected_count:
        raise SystemExit(f"Expected {expected_count} teams, extracted {len(rows)}. Refusing to write.")
...
def main():
    ...
    if os.environ.get("YAHOO_UPSERT_TEAM_MAP", "").strip() == "1":
        ...
        dsn = os.environ.get("MLF_POSTGRES_DSN") or os.environ.get("POSTGRES_DSN")
        if not dsn:
            raise SystemExit("Missing env var MLF_POSTGRES_DSN or POSTGRES_DSN")
